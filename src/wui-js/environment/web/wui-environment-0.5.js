...
class WUIEnvironment {
	...
	constructor(properties = {}) {
		...
		this.#platform = (navigator.userAgentData?.platform || navigator.platform || "").toLowerCase();
		this.#systemName =
			/iPad|iPhone|iPod/.test(this.#userAgent) ? "iOS"
			: /android/i.test(this.#userAgent) ? "Android"
			: this.#platform.includes("mac") ? "macOS"
			: this.#platform.match(/windows phone|windows mobile/) ? "Windows Phone"
			: this.#platform.includes("win") ? "Windows"
			: this.#platform.includes("linux") ? "Linux"
			: "unknown";
		this.#environment =
	...
	isLocalHost() {
		return Boolean(location.hostname === "localhost" || location.hostname === "127.0.0.1");
	}
		...
		if (this.isLocal()) {
			...
			return (async () => {
				...
				try {
					...
					const contactsState = "contacts" in navigator && "ContactsManager" in window ? "prompt" : "undefined";
					...
					permissions.contacts = contactsState;
